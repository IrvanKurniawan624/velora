...
class TimerGuard:
    def __init__(self, max_total_seconds: float = 570.0) -> None:
        ...
        logger.info(f"TimerGuard initialized with {max_total_seconds} seconds total budget.")
    ...
    async def run_with_timeout(self, coro, task_timeout: float = 30.0):
        """Run an async coroutine with a timeout.

        The timeout is the minimum of the task_timeout and the remaining global time budget.
        """
        remaining_time = self.remaining()
        actual_timeout = min(task_timeout, remaining_time)
        ...
        try:
            ...
            logger.error(f"Task timed out after {actual_timeout:.2f}s (global limit check).")
            raise TimeoutError(f"Task timed out after {actual_timeout:.2f}s.")
